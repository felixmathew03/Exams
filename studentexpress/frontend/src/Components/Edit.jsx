...
const Edit = () => {
    ...
    const getStudent=async ()=>{
        const {status,data}=await axios.get(`http://localhost:3000/api/getstudent/${id}`,{Headers:{"Content-Type":"application/json"}});
        console.log(status);
        if(status===200){
          setStudent({...data})
        }
    ...
    const handleSubmit=async (e)=>{
        e.preventDefault();
        const {status,data}=await axios.post(`http://localhost:3000/api/editstudent/${id}`,student,{Headers:{"Content-Type":"application/json"}});
        console.log(status);
        if(status===201){
          alert(data.msg);
          navigate('/')
